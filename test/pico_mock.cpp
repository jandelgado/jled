...
void gpio_set_function(uint gpio, gpio_function_t fn) {
    assert(gState_);
    gState_->gpio_function[gpio] = static_cast<uint32_t>(fn);
}

void pwm_set_wrap(uint slice_num, uint16_t wrap) {
    assert(gState_);
    gState_->slices[slice_num].wrap = wrap;
}
...
void pwm_set_enabled(uint slice_num, bool enabled) {
    assert(gState_);
    gState_->slices[slice_num].enabled = enabled;
}

void pwm_set_chan_level(uint slice_num, uint chan, uint16_t level) {
    assert(gState_);
    gState_->slices[slice_num].chan_level[chan] = level;
}

absolute_time_t get_absolute_time() {
    assert(gState_);
    return gState_->boot_time_us;
}
